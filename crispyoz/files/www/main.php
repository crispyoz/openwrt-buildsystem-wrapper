<?php
require 'inc/config.php';
require 'inc/api.php';
$api = new API();
include("inc/session_check.php");
$api_method = 'POST';

$timezone_saved = false;
$wifi_section_saved = false;
$wifi_details_saved = false;

$device_mac = file_get_contents('/sys/class/net/ra0/address', FALSE, NULL, 0, 17);
$t_mac = explode(':', $device_mac);
$host_name = $t_mac[2] . $t_mac[3] . $t_mac[4] . $t_mac[5];
$redirect_url = str_replace('00000000', $host_name, SITE_URL);

?>

<!DOCTYPE html>
<html lang="en">

    <head>

        <meta charset="utf-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
        <meta name="description" content="">
        <meta name="author" content="">

        <title><?php echo(SYSTEM_NAME)?></title>

        <!-- Custom fonts for this template -->
        <link href="assets/css/all.min.css" rel="stylesheet" type="text/css">

        <!-- Custom styles for this template -->
        <link href="assets/css/bootstrap.min.css" rel="stylesheet">

        <!-- Custom styles for this page -->
        <link href="assets/css/dataTables.min.css" rel="stylesheet">
        <!-- <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.0.3/css/font-awesome.css" rel="stylesheet"> -->

        <link rel="shortcut icon" type="image/x-icon" href="./assets/images/favicon.ico" />

        <?php include("inc/styles.php"); ?>

    </head>

    <body id="page-top">


        <?php
        include("inc/scripts.php"); 
        include_once('./wizard_data.php');
        if($timezoneselected == false) {
            include('wizard1.php');
        } else if($wifiselected == false) {
            $post_data = [
                "jsonrpc" => "2.0",
                "id" => 1,
                "method" => "call",
                "params" => [
                    0 => $_SESSION['ubus_rpc_session'],
                    1 => "iwinfo",
                    2 => "scan",
                    3 => ["device" => "apcli0"],
                ],
            ];
            error_log("Before get wifi");
            error_log(print_r($post_data, TRUE));
            $get_wifi_response = $api->process_api($api_method, json_encode($post_data));
            error_log(print_r($get_wifi_response, TRUE));               
	    if (isset($get_wifi_response['error'])) {
//	    $ssid_list = 
	    }else {
            	$ssid_list = $get_wifi_response['result'][1]['results'];  
            }
            error_log(print_r($ssid_list, TRUE));            

            include('wizard2.php');
        } else if($passwordadded == false) {
            include('wizard3.php');
        } else if($dataconsent == false) {
            include('wizard4.php');
        } else if($acceptlicense == false) {
            include('wizard5.php');
        } else if($finalstep == true && empty($_POST['submitit'])) {
            include('wizard6.php');
        } else if(!empty($_POST['submitit'])) {
            include('wizard6.php');
            //include ('inc/save_settings.php');
        ?>

            <div id="my-ui-block" style="z-index: 999;position: fixed;top: 0;left: 0;background: rgba(0,0,0,0.7);width: 100%;height: 100%;">
                <img style="top: 50%;position: fixed;left: 50%;" src="data:image/gif;base64," />
                <div style=" color: white; font-weight: bolder; position: absolute;top: 61%;left: 50%;transform: translateX(-50%);font-size: 18px;">Please wait, applying your configuration and rebooting......</div>
            </div>
           <script>
                setTimeout(() => {
                        window.location.href = "<?php echo($redirect_url)?>"; /// change url here
                    },<?php echo(REDIRECT_TIMEOUT) ?>); // change time in miliseconds
            </script> 
        <?php } ?>

    </body>

</html>

